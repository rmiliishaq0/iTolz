...
<div class="flex justify-center items-center w-full md:inset-0 h-[calc(100%-1rem)] max-h-full w-full h-svh mt-10">
        ...
        <div class="relative bg-white rounded-lg  dark:bg-gray-600 ">
            ...
            <div class="p-4 md:p-5">
                <form method="POST" class="space-y-4" action="{{ route('password.update') }}">
                    ...
                    <div>
                            <label id="lcpass" for="Confirm Password" class="@error("password_confirmation") text-red-600 block mb-2 text-sm font-medium @else block mb-2 text-sm font-medium text-gray-900 dark:text-white @enderror">Confirm Password</label>
                            <input oninput="change()" type="password" name="password_confirmation" id="password_confirmation" placeholder="••••••••" class="@error("password") bottom-100 bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white dark:border-red-500 border-red-600 dark:focus:border-red-500 focus:outline-none focus:ring-0 focus:border-red-600 m-0 text-red-600 @else bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-indigo-500 focus:border-indigo-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white @enderror" required autocomplete="new-password"/>
                    </div>
                    ...
                    @elseif ($errors->has('password') or $errors->has("password_confirmation") or $errors->has("email"))
                    ...
                    @else
                        <button type="submit" class="w-full text-white bg-indigo-700 hover:bg-indigo-800 focus:ring-4 focus:outline-none focus:ring-indigo-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-indigo-600 dark:hover:bg-indigo-700 dark:focus:ring-indigo-800">Reset Password</button>
                    @endif
                </form>
            </div>
