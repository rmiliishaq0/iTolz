...
<body>
    <div class="fixed right-10 bottom-10">
        <a href="{{ route("product_create") }}" type="button" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm p-2.5 text-center inline-flex items-center me-2 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 text-xl">+</a>
    </div>
    ...
    <table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
        ...
        <tbody>
             @foreach (App\Models\Product::all() as $product)
                ...
                <td class="px-6 py-4">{{ $product->path}}</td>
                <td class="px-6 py-4">{{ $product->data }}</td>
                <td class="px-6 py-4">{{ $product->url}}</td>
                <td class="px-6 py-4">{{ $product->removed}}</td>
                <td class="px-6 py-4">{{ $product->is_local ? "yes" : "no"}}</td>
                    <td class="px-6 py-4">{{ $product->Product_Name}}</td>
                    <td class="px-6 py-4">{{ $product->Product_Description}}</td>
                    <td class="px-6 py-4">{{ $product->Name_Seo}}</td>
                    <td class="px-6 py-4">{{ $product->Description_Seo}}</td>
                    <td class="px-6 py-4">{{ $product->Price}}</td>
                    <td class="px-6 py-4">{{ json_decode( $product->Mode,true)}}</td>
                    <td class="px-6 py-4">{{ $product->Product_Type}}</td>
                    <td class="px-6 py-4">
                    <a href="{{ route("admin_product",$product->id) }}" class="font-medium text-indigo-600 dark:text-indigo-500 hover:underline">Edit</a>
                </td>
                <td class="px-6 py-4">
...
<script>
    ...
    document.getElementById("delete").addEventListener("click",()=>{
        if(x!=null){
            window.location.href=`product/delete/${x}`
        }
    })
</script>
</html>
